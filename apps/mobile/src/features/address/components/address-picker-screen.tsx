...
import { resolveAddress, splitPrediction } from "../lib/resolve-address";
import type { SelectedAddress } from "../types/selected-address";

type AddressPickerScreenProps = {
  onConfirm: (address: SelectedAddress) => Promise<void>;
  isSaving: boolean;
  onReset?: () => void;
  title?: string;
};
...
export function AddressPickerScreen({
  ...
  const canConfirm = useMemo(
    () =>
      ...
      !isLocating &&
      !isSaving,
    [isLocating, isResolving, isSaving, query, selectedAddress],
  );
        ...
        <View className="flex-1">
          <View className="px-6 pt-4">
            <View className="h-9 flex-row items-center justify-center">
              ...
              <Text className="font-semibold text-xl text-primary">{title}</Text>
            </View>
