...
import { buildJobAcceptanceRequiredPush } from "../_shared/notifications.ts";
import { createServiceSupabase, sendPushToUsers } from "../_shared/push.ts";

type NotifyBookingBody = {
  request_id?: string;
};
...
Deno.serve(async (req) => {
  ...
  let payload: NotifyBookingBody;
  try {
    payload = (await req.json()) as NotifyBookingBody;
  } catch {
  ...
  if (requestError) {
    console.error("notify-booking-confirmed request lookup failed:", requestError.message);
    return jsonResponse({ error: "Failed to verify booking" }, 500);
  }

  if (!request || request.customer_id !== user.id || request.status !== "confirmed") {
    return jsonResponse({ error: "Booking not found" }, 404);
  }

  if (!request.assigned_artisan_id) {
    return jsonResponse({ ok: true, sent: 0, removed: 0, failed: 0, failures: [] });
  }

  try {
    const service = createServiceSupabase();
    const push = buildJobAcceptanceRequiredPush(requestId);
    ...
    console.error(
      "notify-booking-confirmed push failed:",
